...
TEST_F(ParserTest, ParseFunctionWithReturnType)
{
    ...
    ASSERT_EQ(functionNode->parameters.size(), 2) << "Function should have two parameters";
    auto param1 = dynamic_cast<FunctionParameterNode *>(functionNode->parameters[0].get());
    ASSERT_NE(param1, nullptr) << "First parameter should not be null";
    ASSERT_EQ(param1->type->typeName, "int") << "First parameter type should be 'int'";
    ASSERT_EQ(param1->name, "a") << "First parameter name should be 'a'";

    auto param2 = dynamic_cast<FunctionParameterNode *>(functionNode->parameters[1].get());
    ASSERT_NE(param2, nullptr) << "Second parameter should not be null";
    ASSERT_EQ(param2->type->typeName, "int") << "Second parameter type should be 'int'";
...
TEST_F(ParserTest, ParseFunctionComplexBody)
{
    ...
    auto functionNode = dynamic_cast<FunctionNode *>(program->children[0].get());
    ASSERT_NE(functionNode, nullptr) << "First child should be a FunctionNode";
    ASSERT_EQ(functionNode->name, "complexFunction") << "Function name should be 'complexFunction'";
    ASSERT_EQ(functionNode->returnType, "void") << "Function return type should be 'void'";
    auto bodyNode = dynamic_cast<BlockStatementNode *>(functionNode->body.get());
    ASSERT_NE(bodyNode, nullptr) << "Function should have a body";

    // Check for variable declaration inside the function body
    ASSERT_GT(bodyNode->statements.size(), 0) << "Function body should have statements";
    auto varDeclNode = dynamic_cast<VariableDeclarationNode *>(bodyNode->statements[0].get());
    ASSERT_NE(varDeclNode, nullptr) << "First statement should be a VariableDeclarationNode";
